...
p.add_argument('--network_scale_mean', type=float, default=0.0, required=False, help='neural network scale mean')
p.add_argument('--network_scale_factor', type=float, default=32.6, required=False, help='neural network scale factor')
p.add_argument('--state_dim', type=int, default=3, required=False, help='state dimension of the system')
opt = p.parse_args()

print("Training begins!")

dataset = dataio.ReachabilityACCSource(numpoints=opt.numpoints, pretrain=opt.pretrain, tMin=opt.tMin, network_mean=opt.network_scale_mean, state_dim = opt.state_dim,
                                          network_factor=opt.network_scale_factor,tMax=opt.tMax, counter_end=opt.counter_end,
                                          pretrain_iters=opt.pretrain_iters,
                                          num_src_samples=opt.num_src_samples, use_cuda=opt.use_cuda)
...
# Define the loss
loss_fn = loss_functions.initialize_hji_acc(dataset)
...
def val_fn(model, ckpt_dir, epoch):
  ...
  for i in range(num_thetas):
    ...
    for j in range(num_times):
      ...
      coords = torch.cat((time_coords, theta_coords, mgrid_coords), dim=1)
      if opt.use_cuda:
...
training.train(model=model, train_dataloader=dataloader, epochs=opt.num_epochs, lr=opt.lr,
               ...
               model_dir=root_path, loss_fn=loss_fn, validation_fn=val_fn, start_epoch=opt.checkpoint_toload, use_cuda=opt.use_cuda,)